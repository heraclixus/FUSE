...
def parse_args(args=None):
    ...
    parser.add_argument('-cpu', '--cpu_num', default=10, type=int, help="used to speed up torch.dataloader") # 1 cpu for debug printing
    ...
    parser.add_argument('--max_steps', default=450000, type=int, help="maximum iterations to train")
    ...
    parser.add_argument('--loss_type', default='cos', type=str,
                        choices=['cos', 'possibility','weighted_cosine',
    ...
    parser.add_argument("--n_partitions", default=1000, type=int)
    parser.add_argument("--n_hidden_layers", default=1, type=int) # nlayers for mlp
    parser.add_argument("--modulelist", action="store_true", help="determine if a list of mlps should be used")
    # for explicit regularization on the weight(punish it to be more uniform)
    parser.add_argument("--regularize_intensity", default=10.0, type=float, help="strength of entropy reg")
    ...
    parser.add_argument("--use_entity_map", action="store_true", help="whether entity embedding or entity map")
    parser.add_argument("--use_partition_net", action="store_true") # default is global weight
    parser.add_argument("--use_volume_weights", action="store_true") # if not specified don't use any volume weight
    parser.add_argument("--use_asymmetry_loss", action="store_true") # use asymmetry based loss
    
    parser.add_argument("--partition_reg_type", default="sigmoid", type=str, 
                        choices=['01', 'softmax', 'sigmoid'],
                        help='ways to regularize partition weights')
    # extra: to test that over-estimation of possibility (positive should be smaller)
    parser.add_argument("--gamma_gap", default=0.0, type=float)
...
def main(args):
    ...
    if args.geo == 'fuzzy':
        model = KGFuzzyReasoning(
            ...
            regularize_entropy=args.regularize_entropy,
            use_entity_map=args.use_entity_map,
            use_partition_net=args.use_partition_net,
            use_volume_weights=args.use_volume_weights,
            use_asymmetry_loss=args.use_asymmetry_loss,
            regularize_intensity=args.regularize_intensity,
            regularize_volume=args.regularize_volume,
            use_cuda=args.cuda,
            ...
            regularizer_setting_partitions={
                'type': args.partition_reg_type,
                'dual': False,                
                'e_layernorm': args.entity_ln_before_reg
            },
