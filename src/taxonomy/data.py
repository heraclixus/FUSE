...
"""
Training Dataset for Taxonomy Dataset, the pipeline in short:
- read the taxonomy file, containing pairs of parent and child concepts. 
- tokenize the parent and child concepts using a tokenizer (Bert).
- returns the token id for the parent, child, negative parent for a triplet-based ranking loss.
"""
class Taxo_Data_Train(Dataset):
    
    def __init__(self, args, tokenizer, mode="train"):
    
        super(Taxo_Data_Train, self).__init__()
    ...
    """
    use a tokenizer to tokenize the taxonomy data 
    """
    ...
    """
    generate token id for parent and child 
    """
    ...
    def __getitem__(self, index):
        return self.generate_parent_child_token_ids(index)
...
"""
class Taxo_Data_Test(Dataset):
    
    def __init__(self,args,tokenizer):
        
        super(Taxo_Data_Test, self).__init__()
        self.args = args
        self.dataset = args.dataset
        print ("Dataset: {}".format(self.dataset))
        self.data = self.__load_data__(self.dataset)
...
def load_data(args, tokenizer,flag):
    ...
    if flag in set(['test','val']) :
        ...
        data_set = Taxo_Data_Test(args,tokenizer)
    else:
        shuffle_flag = True; drop_last = False; batch_size = args.batch_size; 
        data_set = Taxo_Data_Train(args,tokenizer)
