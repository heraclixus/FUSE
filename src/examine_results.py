...
# load embedding
embedding = torch.load(f"embedding_{mode}_{dataset}.pt")
...
child_embs = embedding[train_parent_list]
...
"""
def compute_rank(union_all_children, children, parent_id):
...
def examine_union():
    ...
    for parent in union_pairs.keys():
        ...
        rank, correctness = compute_rank(union_all_children, children, parent)
        mrr += (1/rank)
...
"""
def compute_rank_c(complement_emb, child_id):
...
def examine_complement():
    ...
    for parent in complement_pairs.keys():
        ...
        rank, correctness = compute_rank_c(minus_emb, remaining)
        mrr += (1/rank)
        n_corrects += correctness
    mrr /= n 
    accuracy = n_corrects / n 
