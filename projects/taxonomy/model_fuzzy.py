...
class SimpleFuzzySet(nn.Module):
    ...
    # def parent_child_asym_hamming_loss(self, pos_pair_possibility, child_possibility):
    #     asymmetry_loss = torch.norm(pos_pair_possibility - child_possibility)
    #     return asymmetry_loss
...
""""
Implementation of FuzzQE for KG query answering
but now for taxonomy expansion task
- in this situation, sets and elements have different embeddings. 
- they then use 
"""

class FuzzQE_Taxo(nn.Module):
    
    def __init__(self,args):
        super(FuzzQE_Taxo, self).__init__()
        ...
        # fuzzy logic related operators - for fuzzQE, different representation for child and parent
        # its assumption is on a probabilty space without the partition weights. 
        self.child_regularizer = get_regularizer("sigmoid", args.entity_dim)
        self.parent_regularizer = get_regularizer("sigmoid",  args.entity_dim)
        self.partition_regularizer = get_regularizer(args.partition_reg_type, self.n_partitions)

    
        self.parent_projection = FuzzyMapping(entity_dim=args.entity_dim, hidden_dim=args.hidden_dim,
                                     num_hidden_layers=args.num_hidden_layers,
                                     regularizer=self.parent_regularizer,
                                     n_partitions=args.n_partitions,
                                     modulelist=args.modulelist)
        
        self.child_projection = FuzzyMapping(entity_dim=args.entity_dim, hidden_dim=args.hidden_dim,
                                     num_hidden_layers=args.num_hidden_layers,
                                     regularizer=self.child_regularizer,
                                     n_partitions=args.n_partitions,
                                     modulelist=args.modulelist)
        self.partition_weights = nn.Parameter(torch.ones((self.n_partitions, )))
    ...
    # project
    def project_fuzzyset_parent(self, encode_inputs):
        cls = self.pre_train_model(**encode_inputs)
        cls = self.dropout(cls[0][:, 0, :]) # (B, 768)
        fuzzy_set = self.parent_projection(cls) # (B, d)
        return fuzzy_set
    
    def project_fuzzyset_child(self, encode_inputs):
        cls = self.pre_train_model(**encode_inputs)
        cls = self.dropout(cls[0][:, 0, :]) # (B, 768)
        fuzzy_set = self.child_projection(cls) # (B, d)
        return fuzzy_set
    
     
    def project_fuzzyset(self, encode_inputs, type="parent"):
        return self.project_fuzzyset_child(encode_inputs) if type == "child" else self.project_fuzzyset_parent(encode_inputs)
    
    
    def forward(self,encode_parent=None,encode_child=None,encode_negative_parents=None,flag="train"):
        parent_fuzzyset = self.project_fuzzyset_parent(encode_parent)
        child_fuzzyset = self.project_fuzzyset_child(encode_child)
        neg_parent_fuzzyset = self.project_fuzzyset_parent(encode_negative_parents)
        return self.parent_child_pair_loss(child_fuzzyset, parent_fuzzyset, neg_parent_fuzzyset)        