...
from constants import query_structure_list
...
class KGReasoning(nn.Module):
    ...
    def forward(self, positive_sample, negative_sample, subsampling_weight, batch_queries_dict, batch_idxs_dict, idxs=None):
        if self.geo == 'box':
    ...
    def forward_beta(self, positive_sample, negative_sample, subsampling_weight, batch_queries_dict, batch_idxs_dict):
        ...
        for query_structure_idx in batch_queries_dict:
            if 'u' in self.query_name_dict[query_structure_list[query_structure_idx]] \
                    and 'DNF' in self.query_name_dict[query_structure_list[query_structure_idx]]:
                alpha_embedding, beta_embedding, _ = \
                    self.embed_query_beta(self.transform_union_query(batch_queries_dict[query_structure_idx],
                                                                     query_structure_idx),
                                          self.transform_union_structure(query_structure_idx),
                                          0)
                all_union_idxs.extend(batch_idxs_dict[query_structure_idx])
                all_union_alpha_embeddings.append(alpha_embedding)
                all_union_beta_embeddings.append(beta_embedding)
            else:
                alpha_embedding, beta_embedding, _ = self.embed_query_beta(batch_queries_dict[query_structure_idx],
                                                                           query_structure_list[query_structure_idx],
                                                                           0)
                all_idxs.extend(batch_idxs_dict[query_structure_idx])
    ...
    def transform_union_query(self, queries, query_structure_idx):
        '''
        transform 2u queries to two 1p queries
        transform up queries to two 2p queries
        '''
        if self.query_name_dict[query_structure_list[query_structure_idx]]  == '2u-DNF':
            queries = queries[:, :-1] # remove union -1
        elif self.query_name_dict[query_structure_list[query_structure_idx]]  == 'up-DNF':
            queries = torch.cat([torch.cat([queries[:, :2], queries[:, 5:6]], dim=1), torch.cat([queries[:, 2:4], queries[:, 5:6]], dim=1)], dim=1)
        queries = torch.reshape(queries, [queries.shape[0]*2, -1])
        return queries

    def transform_union_structure(self, query_structure_idx):
        query_structure = query_structure_list[query_structure_idx]
        ...
        for query_structure_idx in batch_queries_dict:
            if 'u' in self.query_name_dict[query_structure_list[query_structure_idx]]:
                center_embedding, offset_embedding, _ = \
                    self.embed_query_box(self.transform_union_query(batch_queries_dict[query_structure_idx],
                                                                    query_structure_idx),
                                         self.transform_union_structure(query_structure_idx),
                                         0)
                ...
                center_embedding, offset_embedding, _ = self.embed_query_box(batch_queries_dict[query_structure_idx],
                                                                             query_structure_list[query_structure_idx],
                                                                             0)
    ...
    def forward_vec(self, positive_sample, negative_sample, subsampling_weight, batch_queries_dict, batch_idxs_dict):
        ...
        for query_structure_idx in batch_queries_dict:
            if 'u' in self.query_name_dict[query_structure_list[query_structure_idx]] and 'DNF' in self.query_name_dict[query_structure_list[query_structure_idx]]:
                center_embedding, _ = self.embed_query_vec(self.transform_union_query(batch_queries_dict[query_structure_idx],
                                                                    query_structure_idx),
                                                                self.transform_union_structure(query_structure_idx), 0)
                all_union_center_embeddings.append(center_embedding)
                all_union_idxs.extend(batch_idxs_dict[query_structure_idx])
            else:
                center_embedding, _ = self.embed_query_vec(batch_queries_dict[query_structure_idx], query_structure_list[query_structure_idx], 0)
                all_center_embeddings.append(center_embedding)
                all_idxs.extend(batch_idxs_dict[query_structure_idx])
    ...
    def test_step(model, easy_answers, hard_answers, args, test_dataloader, query_name_dict, save_result=False, save_str="", save_empty=False):
        ...
        with torch.no_grad():
            for negative_sample, queries, queries_unflatten, query_structure_idxs in tqdm(test_dataloader, disable=not args.print_on_screen):
                ...
                for i, query in enumerate(queries):
                    batch_queries_dict[query_structure_idxs[i]].append(query)
                    batch_idxs_dict[query_structure_idxs[i]].append(i)
                for query_structure_idx in batch_queries_dict:
                    if args.cuda:
                        batch_queries_dict[query_structure_idx] = torch.LongTensor(batch_queries_dict[query_structure_idx]).cuda()
                    else:
                        batch_queries_dict[query_structure_idx] = torch.LongTensor(batch_queries_dict[query_structure_idx])
                if args.cuda:
                    negative_sample = negative_sample.cuda()

                _, negative_scores, _, idxs = model(None, negative_sample, None, batch_queries_dict, batch_idxs_dict)
                queries_unflatten = [queries_unflatten[i] for i in idxs]
                query_structure_idxs = [query_structure_idxs[i] for i in idxs]
                argsort = torch.argsort(negative_scores, dim=1, descending=True)
        ...
        for query_structure_idx in logs:
            for metric in logs[query_structure_idx][0].keys():
                if metric in ['num_hard_answer']:
                    continue
                metrics[query_structure_idx][metric] = sum([log[metric] for log in logs[query_structure_idx]])/len(logs[query_structure_idx])
