...
from models import *
from operations import EntityMapping
import wandb
from constants import query_structure_list, query_structure2idx, loss_type_with_weights
...
class KGFuzzyReasoning(KGReasoning):
    def __init__(
        self, nentity, nrelation, hidden_dim, gamma, gamma_gap,
        geo, n_partitions, modulelist, num_hidden_layers, 
        regularize_entropy, use_entity_map, use_partition_net, 
        use_volume_weights, regularize_intensity, regularize_volume,
        use_asymmetry_loss,
        test_batch_size=1,
        ...
        regularizer_setting_partitions=None,
        gamma_coff=20,
        loss_type='cos',
        ...
        self.entity_dim = hidden_dim  #* 2 # This is temporary
        ...
        self.use_entity_map = use_entity_map
        self.use_partition_net = use_partition_net
        self.regularize_volume = regularize_volume
        self.use_asymmetry_loss = use_asymmetry_loss 
        ...
        self.conjunction_net = Conjunction(self.n_partitions, logic_type, regularizer_setting, use_attention=args.use_attention, godel_gumbel_beta=godel_gumbel_beta)
        self.disjunction_net = Disjunction(self.n_partitions, logic_type, regularizer_setting, godel_gumbel_beta=godel_gumbel_beta)
        self.negation_net = Negation(self.n_partitions, logic_type, regularizer_setting)
        
        
        """
        Study on the impact of volume weights
        """
        # w/o regularization 
        self.partition_weights = nn.Parameter(torch.ones((self.n_partitions, )))
        # 01 regularization
        self.partition_regularizer = get_regularizer(regularizer_setting_partitions, self.n_partitions, 
                                                     neg_input_possible=False, entity=False)
        
        # self.partition_regularizer = nn.Softmax(dim=-1) # temporary change to make plain softmax after the global weights 
        
        self.partition_net = SimpleMLP(input_dim=self.n_partitions, 
                                        hidden_dim=self.hidden_dim, 
                                        num_hidden_layers=1,
                                        regularizer=self.partition_regularizer,
                                        output_dim=n_partitions)
        self.regularize_entropy = regularize_entropy
        self.regularize_intensity = regularize_intensity
    ...
    def forward(
        ...
        # keep track of partition weight when entropy regularization is on
        # this is specifically for the partition_net object
        partition_weight = torch.Tensor([]).to(self.device)
        ...
        # print(batch_idxs_dict)
        # print("--------------------------------------------------------------")

        # sample_idxs[0] as the boolean matching returns a ctually a tuple
        # moreoever, there could be queries of difference sizes in batch queries, causing problems.
        batch_queries_dict = {
        ...
        if positive_sample is not None:
            if len(all_embeddings) > 0:
                ...
                if self.use_entity_map:
                    positive_embedding = self.entity_map(positive_embedding)
                if not self.use_volume_weights:
                    positive_score = self.cal_logit_fuzzy(positive_embedding, all_embeddings, inference=inference)
                elif self.regularize_entropy and self.loss_type in loss_type_with_weights:
                    positive_score, partition_weight_positive, asym_loss_positive = self.cal_logit_fuzzy(positive_embedding, all_embeddings, inference=inference)
                    partition_weight = torch.cat([partition_weight, partition_weight_positive], dim=-1)
                elif self.loss_type in loss_type_with_weights:
                    positive_score, _, asym_loss_positive = self.cal_logit_fuzzy(positive_embedding, all_embeddings, inference=inference)
                else:
                    positive_score = self.cal_logit_fuzzy(positive_embedding, all_embeddings, inference=inference)
            else:
        ...
        if negative_sample is None:
            ...
            if len(all_embeddings) > 0:
                ...
                if self.loss_type.startswith('discrete'):
                    ...
                    else: # with entity map, we don't map entities into embeddings first
                    ...
                    else: # entity map, not using embedding 
                        negative_embedding =  torch.index_select(
                                self.entity_embedding,
                                dim=0,
                                index=negative_sample_regular.view(-1)
                ...
                if self.use_entity_map:
                    negative_embedding = self.entity_map(negative_embedding)
                if not self.use_volume_weights:
                    negative_score = self.cal_logit_fuzzy(negative_embedding, all_embeddings, inference=inference)
                elif self.regularize_entropy and self.loss_type in loss_type_with_weights:
                    negative_score, partition_weight_negative, asym_loss_negative = self.cal_logit_fuzzy(negative_embedding, all_embeddings, inference=inference)
                    partition_weight = torch.cat([partition_weight, partition_weight_negative], dim=-1)
                elif self.loss_type in loss_type_with_weights:
                    negative_score, _, asym_loss_negative = self.cal_logit_fuzzy(negative_embedding, all_embeddings, inference=inference)
                else: # cos 
                    negative_score = self.cal_logit_fuzzy(negative_embedding, all_embeddings, inference=inference)
            else:
        ...
        if self.counter_for_neg and (not inference):
            ...
            if not self.use_volume_weights:
                negative_score_2 = self.cal_logit_fuzzy(positive_embedding, neg_q_embeddings, inference=inference)
            elif self.regularize_entropy and self.loss_type in loss_type_with_weights:
                negative_score_2, partition_weight_positive, asym_loss_positive_2 = self.cal_logit_fuzzy(positive_embedding,
                                                                                   neg_q_embeddings,
                                                                                   inference=inference)
                partition_weight = torch.cat([partition_weight, partition_weight_positive], dim=-1)
            elif self.loss_type in loss_type_with_weights:
                negative_score_2, _, asym_loss_negative_2 = self.cal_logit_fuzzy(positive_embedding, neg_q_embeddings, inference=inference)  # [batch_size, 1]
            else:
            ...
            asym_loss_positive += asym_loss_positive_2
            asym_loss_negative += asym_loss_negative_2
            
        
        if self.regularize_entropy:
            return positive_score, negative_score, asym_loss_positive, asym_loss_negative, partition_weight, subsampling_weight, all_idxs
        else:
            return positive_score, negative_score, asym_loss_positive, asym_loss_negative, subsampling_weight, all_idxs
        ...
        if all_relation_flag:  # only relation traversal
            if query_structure[0] == 'e':
                if self.simplE:
                    ...
                    embedding = torch.index_select(self.entity_head_embedding, dim=0, index=queries[:, idx])
                    ...
                    if self.no_anchor_reg:
                        ...
                        embedding = torch.index_select(self.entity_embedding, dim=0, index=queries[:, idx])
                if self.use_entity_map:
                    embedding = self.entity_map(embedding)
    ...
    """
    specifically for weighted cosine 
    entity_pl_fuzzyset: (b,n,d)
    query_pl_fuzzyset: (b,1,d)    
    """
    def cal_weighted_cosine_possibility(self, entity_pl_fuzzyset, query_pl_fuzzyset, type="pos"):
        ...
        if not self.use_volume_weights:
            score = torch.einsum("bnd,bd->bn", entity_pl_fuzzyset, query_pl_fuzzyset)
            partition_weights = None

        if self.use_partition_net: # not global 
            partition_weights = self.partition_net(query_pl_fuzzyset)
            weighted_entity = torch.einsum("bnd,bd->bnd", entity_pl_fuzzyset, partition_weights)
            weighted_query = query_pl_fuzzyset * partition_weights
        else:
            if self.regularize_volume:
                ...
                partition_weights = self.partition_regularizer(self.partition_weights)
            else:  # global 
        ...
        # print(f"weighted_entity = {weighted_entity.shape}, weighted_query = {weighted_query.shape}")
        score = torch.einsum("bnd,bd->bn", weighted_entity, weighted_query)
        
        asymmetry_loss = None
        
        if self.use_asymmetry_loss:
            if not self.use_volume_weights:
                intersection_pl_fuzzyset = torch.einsum("bnd,bd->bnd", entity_pl_fuzzyset, query_pl_fuzzyset)
                asym_score = intersection_pl_fuzzyset / entity_pl_fuzzyset
            else:
                intersection_pl_fuzzyset = torch.einsum("bnd,bd->bnd", weighted_entity, weighted_query)
                asym_score = intersection_pl_fuzzyset / weighted_entity
            if type == "pos":
                # bnd -> nd -> d 
                asymmetry_loss = torch.mean(torch.mean(torch.square(asym_score - 1), dim=0), dim=0)
            else:
                asymmetry_loss = torch.mean(torch.mean(torch.square(asym_score), dim=0), dim=0)
                
            print(f"asymmtry_loss = {asymmetry_loss}, shape = {asymmetry_loss.shape}")
        
        return score, partition_weights, asymmetry_loss
    
    
    """
    calculate losses associated with possibility-based scores:
    - can be possibility 
    - or weighted cosine score
    entity_pl_fuzzyset: (b,n,d)
    query_pl_fuzzyset: (b,1,d)
    """
    def cal_pl_fuzzy_possibility(self, entity_pl_fuzzyset, query_pl_fuzzyset):
        ...
        if not self.use_volume_weights:
            score = torch.sum(intersection_pl_fuzzyset, dim=-1) # bn
            return score
    
        # possibility score 
        if not self.use_partition_net:
            if self.regularize_volume: # regularize volume 
                score = torch.sum(intersection_pl_fuzzyset * self.partition_regularizer(self.partition_weights), dim=-1).T # (B,n)
                partition_weights = self.partition_regularizer(self.partition_weights)
            else:
                score = torch.sum(intersection_pl_fuzzyset * self.partition_weights, dim=-1).T # (B,n)
                partition_weights = self.partition_weights
        # set-dep partition weights automatically has regularization 
        else:
            partition_weights = self.partition_net(query_pl_fuzzyset) # the simple MLP already has a regularizer
            score = torch.sum(intersection_pl_fuzzyset * partition_weights, dim=-1).T # (B,n) 
        return score, partition_weights
    ...
    def cal_logit_fuzzy(self, entity_embedding, query_embedding, inference=False):
        ...
        """
        if self.use_entity_map:
            entity_embedding = entity_embedding.permute(1,0,2)
        # step 3  change: use possibility as an option for loss function.
        if self.loss_type == "possibility":
            return self.cal_pl_fuzzy_possibility(entity_embedding, query_embedding)   
        
        if self.loss_type == "weighted_cosine":
            return self.cal_weighted_cosine_possibility(entity_embedding, query_embedding)     
        ...
        if self.loss_type == 'dot':
            ...
            cos = nn.CosineSimilarity(dim=-1)
            # print(f"entity_embedding = {entity_embedding.shape}, query_embedding = {query_embedding.shape}")
            score = cos(entity_embedding, query_embedding)
        return score
    ...
    def compute_loss(model, positive_score, negative_score, subsampling_weight, 
        ...
        if model.margin_type == 'logsigmoid':
            ...
            # gamma as margin
            diff = -F.logsigmoid(model.gamma_coff*(positive_score - model.gamma_gap - negative_score))           
        ...
        # this can turn the loss into negative 
        if partition_weight != None:
            norm_weights = torch.norm(partition_weight) / len(partition_weight)
            loss += norm_weights * model.regularize_intensity  # first attempt is to scale down the entropy
            # print(f"positive entropy = {norm_positives}, negative entropy = {norm_negatives}, new loss = {loss}")        
            log["weight_norm"] = norm_weights
        return loss, log
    ...
    def train_step(model, optimizer, train_iterator, args, step):
        ...
        if model.regularize_entropy:
            positive_score, negative_score,  asym_loss_positive, asym_loss_negative, partition_weight, subsampling_weight, _ = model(
                positive_sample,
                negative_sample,
                subsampling_weight,
            ...
            positive_score, negative_score,  asym_loss_positive, asym_loss_negative, subsampling_weight, _ = model(
                positive_sample,
                negative_sample,
                subsampling_weight,
                batch_queries,  # np.array([queries]), won't be split when using multiple GPUs
        ...
        if model.regularize_entropy:
            loss, log = KGFuzzyReasoning.compute_loss(model, positive_score, negative_score, subsampling_weight, partition_weight)
        else:
            loss, log = KGFuzzyReasoning.compute_loss(model, positive_score, negative_score, subsampling_weight)


        print(f"loss = {loss.item()}, asym_loss_positive = {asym_loss_positive.item()}, asym_loss_negative = {asym_loss_negative.item()}")
        # add asymmetry loss
        loss += asym_loss_positive
        loss += asym_loss_negative
    ...
    def test_step(model, easy_answers, hard_answers, args, test_dataloader, query_name_dict, save_result=False, save_str="", save_empty=False):
        ...
        with torch.no_grad():
            for negative_sample, queries, queries_unflatten, query_structure_idxs in tqdm(test_dataloader, disable=not args.print_on_screen):
                ...
                if model.regularize_entropy:
                    _, negative_logit, _, _, idxs = model(
                    None,
                    negative_sample,
                    None,
